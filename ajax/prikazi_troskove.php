<?php
function prikazi_troskove($pocetni_datum, $krajnji_datum)
{

    //UKLJUCIVANJE KONEKCIJE KA AMSO BAZI
    require_once '../dodaci/konekcija_amso.php';

    //PRETVARANJA POCETNOG I KRAJNJEG DATUMA ZA 4 KVARTALA U NIZOVE
    $niz_datum = explode('-', $pocetni_datum);
    $niz_datum2 = explode('-', $krajnji_datum);
  
    //IZDVAJANJE POCETNE I KRAJNJE GODINE I UPIS U PROMENJIVE
    $pocetna_godina = substr($pocetni_datum, 0, 4);
    $krajnja_godina = substr($krajnji_datum, 0, 4);
...
if (isset($_POST['pocetni_datum'])) {
    ...
    prikazi_troskove($_POST['pocetni_datum'], $_POST['krajnji_datum']);
}